...
type intentTrainingItem struct {
	id   uint8
	data string
}

func TrainIntent(trainingDataFilePath, mapOutputFile, modelOutputFile string) error {
	...
	trainingData := make([]intentTrainingItem, 0)
	...
	for {
		...
		trainingData = append(trainingData, intentTrainingItem{
			id:   id,
			data: record[1],
		})
	}
